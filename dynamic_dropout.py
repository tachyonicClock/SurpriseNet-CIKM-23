...
from helper import get_eval_plugin, tb_logger
...
x=0.3
cond_dropout = ConditionedDropout(512, 5, x, 0.55)
naive_dropout = NaiveDropout()
model = SimpleDropoutMLP(num_classes=scenario.n_classes, hidden_layers=1, dropout_module=cond_dropout)

eval_plugin = get_eval_plugin([tb_logger(f"cond_dropout_{x}"), TextLogger(open('log.txt', 'a')), InteractiveLogger()], scenario.n_classes)

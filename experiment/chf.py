...
def _split_experience(
    experience: NCExperience, split_proportion: float
    ...
    train_exp = NCExperience(experience.origin_stream, experience.current_experience)
    val_exp = NCExperience(experience.origin_stream, experience.current_experience)
...
class CHF_SurpriseNet(Strategy):
    ...
    def search_lr(
        self, train_exp: NCExperience, valid_exp: NCExperience, **kwargs
    ) -> float:
    ...
    def train(self, experiences, eval_streams=None, **kwargs):
        ...
        lr, reference_accuracy = self.search_lr(train_exp, valid_exp, **kwargs)
        self.optimizer.param_groups[0]["lr"] = lr

        # Use Stability Decay Search to find the best pruning proportion
        self.search_tradeoff(train_exp, valid_exp, reference_accuracy, **kwargs)
